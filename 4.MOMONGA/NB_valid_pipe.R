...
NB_tumor <- readRDS("/cluster3/yflu/STS/Drug_screen/NB_tumor_aucell_druggable_genesets.rds")
samplenames <- unique(as.character(NB_tumor$Sample))

cnv_genes <- readRDS("/cluster3/yflu/STS/Drug_screen/NB/cnv_genes.rds")
...
degs_tumor_normal_all <- readRDS("/cluster3/yflu/STS/Drug_screen/NB_tumor_markers.rds")
degs_tumor_normal_all <- subset(degs_tumor_normal_all,avg_log2FC > 0&p_val_adj < 0.05&cluster == "Tumor")

regulon_targets <- read.csv("/cluster3/yflu/STS/Drug_screen/NB/scenic/reg.csv")
colnames(regulon_targets) <- c(regulon_targets[2,c(1,2)],regulon_targets[1,-c(1,2)])
regulon_targets <- regulon_targets[-c(1,2),]

#NB_sample_markers <- readRDS("/cluster3/yflu/STS/Drug_screen/NB_sample_markers.rds")
...
for (i in 1:length(samplenames)) {
  ...
  print(paste(i,"/57",sep = ""))
}
saveRDS(cnv_genes_top_list,"/cluster3/yflu/STS/Drug_screen/NB_cnv_genes_top_list_251204.rds")
saveRDS(CNV_correlated_drugs_list,"/cluster3/yflu/STS/Drug_screen/NB_CNV_correlated_drugs_list_251204.rds")

aucell_druggable_genesets <- readRDS("/cluster3/yflu/STS/Drug_screen/NB_aucell_druggable_genesets.rds")
aucell_druggable_genesets <- t(as.matrix(aucell_druggable_genesets))
aucell_druggable_genesets_scaled <- readRDS("/cluster3/yflu/STS/Drug_screen/NB_aucell_druggable_genesets_scaled.rds")
...
for (nm in names(CNV_aucell_drugs_top_list)) {
  
  item <- CNV_aucell_drugs_top_list[[nm]]
...
saveRDS(CNV_aucell_drugs_top_list,"NB_CNV_aucell_drugs_top_list_251204.rds")

drug_hgnc_list_combined <- readRDS("/cluster3/yflu/STS/Drug_screen/drug_hgnc_list_combined.rds")
NB_CNV_aucell_drugs_top_list_251204 <- readRDS("/cluster3/yflu/STS/Drug_screen/NB_CNV_aucell_drugs_top_list_251204.rds")

drug_count <- list()   # 保存每个样本的计数
drug_global_count <- list()  # 所有样本的总体计数

for (nm in names(NB_CNV_aucell_drugs_top_list_251204)) {
  
  item <- NB_CNV_aucell_drugs_top_list_251204[[nm]]
...
CNV_aucell_drugs_merged_sub_1 <- CNV_aucell_drugs_merged_sub[,-c(1,59)]
...
p <- ggplot(top_gene_per_drug2_unique, aes(x = chr, y = Freq)) +
  geom_col(fill = "steelblue", width = 0.7) +
  
  # 标注 Freq 前 10 的 drug_short
  geom_text(
    data = top10_label,
    aes(label = drug_short),
    vjust = -0.5,
    size = 3,
    angle = 90
  ) +
  ...
  labs(
    x = "Chromosome",
    y = "Frequency",
    title = "Genome-wide distribution of drug-associated genes"
  ) +
