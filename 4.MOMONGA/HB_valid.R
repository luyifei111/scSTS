...
library(aplot)
library(stringr)
base_dir <- "/cluster3/yflu/STS/Drug_screen/HB"

# 列出所有一级子目录（假设每个样本一个文件夹）
subdirs <- list.files(base_dir, full.names = TRUE, recursive = FALSE)
subdirs <- subdirs[-c(1:5)]
samples <- substr(subdirs,35,nchar(subdirs))
samples <- substr(samples,1,nchar(samples)-23)

saveRDS(samples,"/cluster3/yflu/STS/Drug_screen/HB/samples_hb.rds")

HB <- readRDS("/cluster3/yflu/STS/Drug_screen/HB/9celltype_HB.rds")
HB_tumor <- subset(HB,celltype_new %in% c("hepatic","mesenchymal")) 
HB_tumor <- subset(HB_tumor,sample %in% samples)
...
HB_tumor <- irGSEA.score(object = HB_tumor, assay = "RNA", slot = "data", 
                         seeds = 123, ncores = 20,msigdb=F, 
                         custom = T, geneset = drug_hgnc_list_combined, method = c("AUCell", "UCell", "singscore",
                                                                                   "ssgsea")[c(1)], 
                         kcdf = 'Gaussian')
saveRDS(HB_tumor,"/cluster3/yflu/STS/Drug_screen/HB_tumor_aucell_druggable_genesets.rds")

aucell_drugs <- AverageExpression(HB_tumor,assays = "AUCell",group.by = "sample")
aucell_drugs <- aucell_drugs$AUCell
saveRDS(aucell_drugs,"/cluster3/yflu/STS/Drug_screen/HB_aucell_druggable_genesets.rds")
aucell_drugs_scaled <- t(aucell_drugs)
aucell_drugs_scaled <- scale(aucell_drugs_scaled)
saveRDS(aucell_drugs_scaled,"/cluster3/yflu/STS/Drug_screen/HB_aucell_druggable_genesets_scaled.rds")

#HB_sample_markers <- FindAllMarkers(HB_tumor,group.by = "sample")

HB$group <- ifelse(HB$celltype_new %in% c("hepatic","mesenchymal"), "Tumor", "Normal")
HB_tumor_markers <- FindAllMarkers(HB,group.by = "group")

#saveRDS(HB_sample_markers,"/cluster3/yflu/STS/Drug_screen/HB_sample_markers.rds")
saveRDS(HB_tumor_markers,"/cluster3/yflu/STS/Drug_screen/HB_tumor_markers.rds")

samples <- readRDS("/cluster3/yflu/STS/Drug_screen/HB/samples_hb.rds")
CNV_region_path <- paste("/cluster3/yflu/STS/Drug_screen/HB/hmm/",samples,"_HMM_CNV_predictions.HMMi6.hmm_mode-samples.Pnorm_0.5.pred_cnv_regions.dat",sep = "")
infercnv_gene_order_path <- paste("/cluster3/yflu/STS/Drug_screen/HB/",samples,"_run.final.infercnv_obj",sep = "")
...
saveRDS(cnv_genes,"/cluster3/yflu/STS/Drug_screen/HB/cnv_genes.rds")
