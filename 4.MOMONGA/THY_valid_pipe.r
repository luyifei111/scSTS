...
counts <- read.delim("/cluster3/yflu/STS/Drug_screen/pan-cancer/GSE132465_GEO_processed_THY_10X_raw_UMI_count_matrix.txt")
rownames(counts) <- counts$Index
counts <- counts[,-1]
THY <- CreateSeuratObject(counts)

meta <- read.delim("/cluster3/yflu/STS/Drug_screen/pan-cancer/GSE132465_GEO_processed_THY_10X_cell_annotation.txt")
rownames(meta) <- meta$Index
rownames(meta) <- gsub("-", ".", rownames(meta))

THY@meta.data <- meta[colnames(THY),]
THY <- subset(THY,Class == "Tumor")

THY_tumor <- subset(THY,Cell_type %in% c("Epithelial cells"))

samplenames <- readRDS("/cluster3/yflu/STS/Drug_screen/THY/samples_THY.rds")

cnv_genes <- readRDS("/cluster3/yflu/STS/Drug_screen/THY/cnv_genes.rds")
...
degs_tumor_normal_all <- readRDS("/cluster3/yflu/STS/Drug_screen/THY_tumor_markers.rds")
degs_tumor_normal_all <- subset(degs_tumor_normal_all,avg_log2FC > 0&p_val_adj < 0.05&cluster == "Tumor")

regulon_targets <- read.csv("/cluster3/yflu/STS/Drug_screen/THY/scenic/reg.csv")
colnames(regulon_targets) <- c(regulon_targets[2,c(1,2)],regulon_targets[1,-c(1,2)])
regulon_targets <- regulon_targets[-c(1,2),]

#THY_sample_markers <- readRDS("/cluster3/yflu/STS/Drug_screen/THY_sample_markers.rds")
...
saveRDS(cnv_genes_top_list,"/cluster3/yflu/STS/Drug_screen/THY_cnv_genes_top_list_260108.rds")
saveRDS(CNV_correlated_drugs_list,"/cluster3/yflu/STS/Drug_screen/THY_CNV_correlated_drugs_list_260108.rds")

aucell_druggable_genesets <- readRDS("/cluster3/yflu/STS/Drug_screen/THY_aucell_druggable_genesets.rds")
aucell_druggable_genesets <- t(as.matrix(aucell_druggable_genesets))
aucell_druggable_genesets_scaled <- readRDS("/cluster3/yflu/STS/Drug_screen/THY_aucell_druggable_genesets_scaled.rds")
...
saveRDS(CNV_aucell_drugs_top_list,"THY_CNV_aucell_drugs_top_list_260108.rds")

THY_CNV_aucell_drugs_top_list_260108 <- readRDS("/cluster3/yflu/STS/Drug_screen/THY_CNV_aucell_drugs_top_list_260108.rds")
THY_cnv_genes_top_list_260108 <- readRDS("/cluster3/yflu/STS/Drug_screen/THY_cnv_genes_top_list_260108.rds")

drug_count <- list()   # 保存每个样本的计数
drug_global_count <- list()  # 所有样本的总体计数

for (nm in names(THY_CNV_aucell_drugs_top_list_260108)) {
  
  item <- THY_CNV_aucell_drugs_top_list_260108[[nm]]
...
# 使用
CNV_aucell_drugs_merged <- combine_aucell_drugs_keep_rownames_full(THY_CNV_aucell_drugs_top_list_260108)
...
for (drug in names(row_nonzero_cols)) {
  ...
  for (sample in samples_with_value) {
    ...
    if (!is.null(THY_cnv_genes_top_list_260108[[sample]])) {
      sample_genes <- THY_cnv_genes_top_list_260108[[sample]]$genes
      # 取交集
...
drug_sample_gene_intersect_updated <- lapply(names(drug_sample_gene_intersect), function(drug) {
  ...
  lapply(names(sample_list), function(sample_id) {
    ...
    if (sample_id %in% names(THY_cnv_genes_top_list_260108)) {
      cnv_df <- THY_cnv_genes_top_list_260108[[sample_id]]
